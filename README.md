...
动作系统 V3.3 的新老对比、实机舵机映射、29个互动动作变体、PCM实时声音驱动、动作内部随机、完整高低尾姿和多层防复读说明见
[docs/elegant_motion_v2.md](docs/elegant_motion_v2.md)。

逐一说明全部29个互动动作、5个长环境音场景、待机动作和调用方式的完整目录见
[docs/dinosaur_action_catalog_v2_8.md](docs/dinosaur_action_catalog_v2_8.md)。
...
| DROOP | 10 | 15 | 5.0s | 尾巴下垂（IO8最小限制为55°） |
...
每个舵机配有独立滑块，实时预览角度值并以40ms节流发送；IO8尾巴上下滑块限制为55°～180°，其余为0°～180°。包含中位、IO17仰天/低头、IO15转头、尾巴上翘/下垂等快捷预设，电池状态每5秒刷新。
